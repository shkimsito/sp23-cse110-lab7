...
async function getRecipes() {
  ...
  if (localStorage.getItem("recipes")){
    return JSON.parse(localStorage.getItem("recipes")); // If there are recipes, return them.
  }
  /**************************/
  // Rest will be concerned with requesting the recipes from the network
    ...
    RECIPE_URLS.forEach(async (recipeUrl) => {
      ...
      try {
        ...
        if (recipes.length == RECIPE_URLS.length){
          saveRecipesToStorage(recipes);  // Then save recipes to storage.
          resolve(recipes);               // Then pass recipes to Promise's resolve()
        }
      } catch (e) {
